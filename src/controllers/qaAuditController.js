...
export async function list(req, res) {
  ...
  if (q)         {
    where.push(`(u.content ILIKE $${i} OR (a.content ILIKE $${i}))`);
    params.push(`%${q}%`); i++;
  }
  if (flagged === '1' || flagged === 'true') {
    where.push(`rv.flagged = TRUE`);
  }
  ...
  try {
    ...
    logAccess(req, 'list', null, { count: items.length, filters: { user_id, agent_id, q, from, to, flagged } });
